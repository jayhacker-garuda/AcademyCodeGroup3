        ...
        <div class=" mt-8 ">
            <div class="relative py-3 w-11/12 max-w-xl sm:mx-auto">
                <div class=" p-8 bg-white shadow-sm sm:rounded-xl">
                    ...
                    <form class="w-full " method="POST" action="{{ route('Admin.store.courses') }}">
                        @csrf
                        <div class="mb-5 relative">
                            <input type="text" id="courses_name" name="courses_name" class="peer pt-8 border border-gray-200 focus:outline-none rounded-md focus:border-gray-500 focus:shadow-sm w-full p-3 h-16 placeholder-transparent" autocomplete="off" />
                        ...
                        <div class="mb-5 relative">
                            <select name="category" id="category" class="peer pt-8 border border-gray-200 focus:outline-none rounded-md focus:border-gray-500 focus:shadow-sm w-full p-3  placeholder-transparent" >
                                <option disabled selected> Courses Category Here </option>
                                @foreach ($categories as $category)
                                    <option value="{{ $category->id }}">{{ $category->course_type }}</option>
                                @endforeach
                            </select>
                        ...
                        <button class="w-full bg-indigo-600 text-white p-3 rounded-md">Create Course</button>
                    </form>
                </div>
            </div>
        </div>
